...
echo 'Program start at: '.date('Y-m-d H:i:s',time())."<br>";
...
{"disease_id":
	...
	"symptoms":
		{"symptom_id":
			...
			"symptom_name":"xxxxx",
			"concepts":
				{"concept_id":"xxxxxx","concept_id":"xxxxxx"}
			}
		},
	"results":{"disease_id":0.56,"disease_id":0.36,"disease_id":0.16,"disease_id":0.02},
	"accuracy":0.56*0.36*0.16*0.02 总分。如果results数目大于5，accuracy=0，小于5的时候，计算乘积
	}
}*/
...
function getAllDiseases(){
	...
	global $userInputs;
	$diseases = array();
	$sql='SELECT site_id,icd,name_decode FROM disease WHERE site_id="DS00216"';//limit 850,30';
	$result = $db->query($sql);
	if($result){
...
function getConceptsFromSymptom($disease_id,$symptom_id){
	...
	getConceptsFromDiseaseJson($disease_id);
}
function getConceptsFromDiseaseJson($disease_id){
	global $userInputs,$sid;
	$concepts = array();
	...
	*/
	...
	$result = file_get_contents('http://'.$hostname.'/explanationWord/API/concept_disease.showfinalDiseaseDetails.php?sid='.$sid);
	
	//echo $result;
	
	$symptom_diseases = json_decode($result,true);
