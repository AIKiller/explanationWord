<?php
	...
	$N = 5; //The variable is used to control the number of output diseases.
	...
	foreach($systemDiseaseSiteIdsObj as $main_site_id => $main_disease){
		...
		$diseaseSimilarities = getDiseaseSimilarties($main_site_id,$main_disease);
		arsort($diseaseSimilarities);
		echo "<b>Disease name:</b>(".$main_disease["icpc"].")".$main_disease["name_decode"]."<br>";
		$i = 0;
		if(count($diseaseSimilarities)==0){
			echo "<h2>The disease does not have relationship symptoms!</h2>";
			continue;
		}
		//next($diseaseSimilarities);//跳过一个疾病，即疾病本身。
		while($i<$N){
	...
	function getDiseaseSimilarties($main_site_id,$main_disease){
		...
		foreach($system_diseases as $similar_site_id => $similar_disease){
			if(trim($main_site_id) == trim($similar_site_id)){
				continue;
			}
